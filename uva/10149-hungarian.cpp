...
int main(){
	...
	int cost[13][13];
	//since this is a maximization method,
	//have to score the negatives of all values
	//returned.
	
	for(int i=0;i<13;i++)
		for(int j=0;j<13;j++){
			cost[i][j] = -1 * score(roll, i ,j);
		}
	...
	for(int i=0;i<13;i++){
		mincost = INFTY;
		for(int j=0;j<13;j++){
			mincost = std::min(mincost, cost[j][i]);
		}
		if(mincost){
			for(int j=0;j<13;j++){
				cost[j][i] -= mincost;
	...
	//now go in a loop until the maxflow returns 13.
	//1. find max flow, if 13, break;
	//2. since not 13, find the 

	//need to minimize this cost matrix.
